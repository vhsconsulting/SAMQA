...
-- changeset SAMQA:1754374130785 stripComments:false logicalFilePath:BASE_RELEASE\samqa\package_bodies\soap_api.sql runAlways:false runOnChange:false replaceIfExists:true failOnError:true
-- sqlcl_snapshot src/database/samqa/package_bodies/soap_api.sql:null:e1dd612c8f389f63d69cc4473973b067b1f7e92f:create

create or replace package body samqa.soap_api as
-- --------------------------------------------------------------------------
    ...
    begin
        ...
        if ( l_fault_node is not null ) then
            l_fault_code := l_fault_node.extract('/'
                                                 || p_response.envelope_tag
                                                 || ':Fault/faultcode/child::text()', 'xmlns:'
                                                                                      || p_response.envelope_tag
                                                                                      || '="http://schemas.xmlsoap.org/soap/envelope/'
                                                                                      ).getstringval();
            ...
            l_fault_string := l_fault_node.extract('/'
                                                   || p_response.envelope_tag
                                                   || ':Fault/faultstring/child::text()', 'xmlns:'
                                                                                          || p_response.envelope_tag
                                                                                          || '="http://schemas.xmlsoap.org/soap/envelope/'
                                                                                          ).getstringval();
