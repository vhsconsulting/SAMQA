...
-- changeset SAMQA:1754373948941 stripComments:false logicalFilePath:BASE_RELEASE\samqa\package_bodies\as_sftp.sql runAlways:false runOnChange:false replaceIfExists:true failOnError:true
-- sqlcl_snapshot src/database/samqa/package_bodies/as_sftp.sql:null:6ccb2598c94c8f89cd287d835788fbbef6e471f5:create

create or replace package body samqa.as_sftp is
  --
    type tp_mag is
  ...
  --
    type tp_name_list is
        table of varchar2(100 char);
  --
    function mag (
        p1 varchar2
    ) return tp_mag;
...
--
  ...
  --
    ...
    begin
        if g_log_level between 1 and 3 then
            log(p_msg);
        end if;
    end;
  --
    ...
    begin
        if g_log_level between 1 and 2 then
            log(p_msg);
        end if;
    end;
  --
  ...
  --
    ...
    begin
        ...
        t1 := lpad(t1, l * ccc, '0');
        for i in 0..l - 1 loop
            n := to_number ( substr(t1, 1 + i * ccc, ccc),
            cfmt1 );
...
--
  ...
  --
    ...
    begin
        ...
        for i in p2.count..p1.count - 1 loop
            t := p1(i) - b;
            if t < 0 then
                b := 1;
                t := t + cm;
  ...
  --
    function rmul (
        ...
        m  tp_mag;
    begin
        for i in 0..y.count + x.count - 2 loop
            m(i) := 0;
        end loop;

        for yi in 0..y.count - 1 loop
            c := 0;
            for xi in 0..x.count - 1 loop
                ci := xi + yi;
                t := m(ci) + x(xi) * y(yi) + c;
  ...
  --
    ...
    --
        function ge (
            ...
            l_ge boolean := true;
        begin
            for i in reverse p..p + y.count - 1 loop
                case sign(x(i) - y(i - p))
                    when 1 then
                        exit;
                    when -1 then
  ...
  --
    begin
      ...
      --
            if ge(rv, ly, xc - yc) then
                sub(rv, ly, xc - yc);
            end if;
      --
            d2 := ly(yc - 1) * cm + ly(yc - 2);

            for i in reverse yc..xc - 1 loop
                if rv(i) > 0 then
                    if rv(i) > d2 then
                        d := cm - 1;
                    else
    ...
    --
            ...
            r := 0;
            if d > 1 then
                for i in reverse 0..x.count - 1 loop
                    tmp := r * cm + x(i);
                    r := tmp - trunc(tmp / d) * d;
                end loop;
            end if;
        ...
        if sf is not null then
            r := 0;
            for i in reverse 0..rv.count - 1 loop
                tmp := rv(i) + r * cm;
                rv(i) := trunc(tmp / sf);
                r := tmp - rv(i) * sf;
            end loop;
  ...
  --
    ...
    begin
        if p1.count = 1 then
            ...
            c := 0;
            for i in reverse 0..p1.count - 1 loop
                t := p1(i) + c;
                rv(i) := trunc(t / 2);
                c :=
                    case
                        when bitand(t, 1) = 1 then
                            cm
                        else 0
                    end;
  ...
  --
    function powmod (
        ...
        begin
            ta := pa;
            for i in ta.count..mc - 1 loop
                ta(i) := 0;
            end loop;

            tb := pb;
            for i in tb.count..mc - 1 loop
                tb(i) := 0;
            end loop;
        ...
        -- check for overflow of t(1)?
            ...
            t.delete(mc + 1);
            for j in reverse 1..t.count - 1 loop
                exit when t(j) > 0;
                t.delete(j);
            end loop;
  ...
  --
    procedure get_named_ed_curve (
        p_name  in varchar2,
        p_curve out tp_ed_curve
    ) is
    begin
        if p_name = 'ed25519' then
            p_curve.b := 256;
...
');
  ...
  --
    procedure get_named_curve (
        p_name  in varchar2,
        p_curve out tp_ec_curve
    ) is
    begin
        if p_name = 'nistp256' then
            p_curve.nlen := 32;
  ...
  --
    procedure bytes_to_ec_point (
        p_bytes raw,
        p_curve tp_ec_curve,
        p_point out tp_ec_point
    ) is
        l_first varchar2(2);
        l_y2    tp_mag;
    begin
        ...
        if not ( (
            l_first = '04'
            and utl_raw.length(p_bytes) = 1 + 2 * p_curve.nlen
        )
        or (
            l_first in ( '02', '03' )
            and utl_raw.length(p_bytes) = 1 + p_curve.nlen
        ) ) then
      ...
      -- see https://tools.ietf.org/id/draft-jivsov-ecc-compact-05.html
            ...
            if l_first = '02' then
                ...
                p_point.y := rsub(p_curve.prime,
                                  powmod(l_y2, p_curve.p_plus_1_div_4, p_curve.prime));
            end if;
  ...
  --
    function from_jacobian (
        p_point tp_ec_point,
        p_curve tp_ec_curve
    ) return tp_ec_point is
        l_inv tp_mag;
        l_tmp tp_mag;
        l_rv  tp_ec_point;
    begin
        if
            p_point.z(0) = 1
            and p_point.z.count = 1
        then
            l_rv.x := p_point.x;
            l_rv.y := p_point.y;
        elsif
            p_point.y(0) = 0
            and p_point.y.count = 1
        then -- infinity
            l_rv.x := c_mag_0;
            l_rv.y := c_mag_0;
        else
            l_inv := powmod(p_point.z,
                            nsub(p_curve.prime, 2),
                            p_curve.prime);

            l_tmp := mulmod(l_inv, l_inv, p_curve.prime);
            l_rv.x := mulmod(p_point.x, l_tmp, p_curve.prime);
            l_rv.y := mulmod(p_point.y,
                             mulmod(l_tmp, l_inv, p_curve.prime),
                             p_curve.prime);
  ...
  --
    ...
    begin
        ...
        then -- infinity
            ...
            l_rv.y := submod(
                mulmod(l_m,
                       submod(l_s, l_rv.x, c.prime),
                       c.prime),
                small_nmulmod(
                             mulmod(l_ysqr, l_ysqr, c.prime),
                             8,
                             c.prime
                         ),
                c.prime
  ...
  --
    ...
    begin
        ...
        l_s1 := mulmod(p1.y,
                       mulmod(l_p2z_pwr2, p2.z, c.prime),
                       c.prime);

        l_s2 := mulmod(p2.y,
                       mulmod(l_p1z_pwr2, p1.z, c.prime),
                       c.prime);

        if requal(l_u1, l_u2) then
            if requal(l_s1, l_s2) then
  ...
  --
    ...
    begin
        ...
        then
            l_rv := p;
        elsif r_greater_equal(m, c.group_order) then
            l_rv := multiply_jpoint(p,
                                    xmod(m, c.group_order),
                                    c);
        elsif bitand(
            m(0),
            1
  ...
  --
    procedure init_hmac_ids is
    begin
        execute immediate 'begin :x := dbms_crypto.HMAC_MD5; end;'
            using out hmac_md5;
        execute immediate 'begin :x := dbms_crypto.HMAC_SH1; end;'
            using out hmac_sh1;
        execute immediate 'begin :x := dbms_crypto.HASH_MD5; end;'
            using out hash_md5;
        execute immediate 'begin :x := dbms_crypto.HASH_SH1; end;'
            using out hash_sh1;
        execute immediate 'begin :x := dbms_crypto.HMAC_SH256; end;'
            using out hmac_sh256;
        execute immediate 'begin :x := dbms_crypto.HMAC_SH512; end;'
            using out hmac_sh512;
        execute immediate 'begin :x := dbms_crypto.HASH_SH256; end;'
            using out hash_sh256;
        execute immediate 'begin :x := dbms_crypto.HASH_SH384; end;'
            using out hash_sh384;
        execute immediate 'begin :x := dbms_crypto.HASH_SH512; end;'
            using out hash_sh512;
    exception
        when others then
            null;
    end;
  --
    ...
    begin
        if g_encr_algo_c in ( 'aes128-cbc', 'aes128-ctr', 'aes192-cbc', 'aes192-ctr', 'aes256-cbc',
                              'aes256-ctr', 'rijndael256-cbc', 'rijndael-cbc@lysator.liu.se' ) then
            l_block_size := 16;
        else
            l_block_size := 8;
        end if;
        ...
        l_padding_len := l_block_size - mod(4 -- size of package length
         + 1 -- size of padding length
         + l_packet_len -- size of payload
         + l_padding_len, l_block_size) + l_padding_len;

        l_packet_len := l_packet_len + l_padding_len + 1;
        l_buf := utl_raw.concat(
            to_char(l_packet_len, 'fm0XXXXXXX'),
            to_char(l_padding_len, 'fm0X'),
            p_buf,
            dbms_crypto.randombytes(l_padding_len)
        );

        if g_mac_algo_c in ( 'hmac-sha1', 'hmac-sha1-96' ) then
            l_hash_type := hmac_sh1;
        elsif g_mac_algo_c in ( 'hmac-md5', 'hmac-md5-96' ) then
            l_hash_type := hmac_md5;
        elsif g_mac_algo_c in ( 'hmac-sha2-256' ) then
            l_hash_type := hmac_sh256;
        elsif g_mac_algo_c in ( 'hmac-sha2-512' ) then
            l_hash_type := hmac_sh512;
        end if;

        if l_hash_type is not null then
            l_mac := dbms_crypto.mac(
                utl_raw.concat(
                    to_char(g_seqn_c, 'fm0XXXXXXX'),
                    l_buf
                ),
                l_hash_type,
                g_key_mac_c2s
            ...
            if g_mac_algo_c in ( 'hmac-md5-96', 'hmac-sha1-96' ) then
                l_mac := utl_raw.substr(l_mac, 1, 12);
            end if;

        end if;

        if g_encr_algo_c in ( 'aes128-cbc', 'aes192-cbc', 'aes256-cbc', 'rijndael256-cbc', 'rijndael-cbc@lysator.liu.se' ) then
            l_buf := dbms_crypto.encrypt(l_buf, dbms_crypto.encrypt_aes + dbms_crypto.chain_cbc + dbms_crypto.pad_none, g_key_cypher_c2s
            , g_iv_cypher_c2s);

            g_iv_cypher_c2s := utl_raw.substr(l_buf, -l_block_size);
        elsif g_encr_algo_c = '3des-cbc' then
            l_buf := dbms_crypto.encrypt(l_buf, dbms_crypto.encrypt_3des + dbms_crypto.chain_cbc + dbms_crypto.pad_none, g_key_cypher_c2s
            , g_iv_cypher_c2s);

            g_iv_cypher_c2s := utl_raw.substr(l_buf, -l_block_size);
        elsif g_encr_algo_c in ( 'aes128-ctr', 'aes192-ctr', 'aes256-ctr' ) then
            for i in 0..trunc(l_packet_len / l_block_size) loop
                l_encr := utl_raw.concat(l_encr,
                                         utl_raw.bit_xor(
                                                     utl_raw.substr(l_buf, i * l_block_size + 1, l_block_size),
                                                     dbms_crypto.encrypt(g_iv_cypher_c2s, dbms_crypto.encrypt_aes + dbms_crypto.chain_cbc
                                                     + dbms_crypto.pad_none, g_key_cypher_c2s)
                                                 ));

                g_iv_cypher_c2s := to_char(
                    nvl(to_number(
                        nullif(g_iv_cypher_c2s,
                               rpad('F', 32, 'F')),
                        rpad('0', 32, 'X')
                    ) + 1,
                        0),
        ...
        l_dummy := utl_tcp.write_raw(
            c    => g_con,
            data => l_buf
        );
        if g_mac_algo_c is not null then
            l_dummy := utl_tcp.write_raw(
                c    => g_con,
                data => l_mac
            );
...
--
    function decrypt (
        ...
        l_type pls_integer;
    begin
        if g_encr_algo_s is null
           or g_encr_algo_s = 'none' then
            return p_encr;
        elsif g_encr_algo_s in ( 'aes128-ctr', 'aes192-ctr', 'aes256-ctr' ) then
            l_tmp := dbms_crypto.encrypt(
                substr(
                    to_char(g_iv_cypher_s2c_ctr, 'FM0XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'),
                    -32
                ),
            ...
            if g_iv_cypher_s2c_ctr > 340282366920938463463374607431768211455 then
                g_iv_cypher_s2c_ctr := 0;
            end if;
            return utl_raw.bit_xor(l_tmp, p_encr);
        end if;

        if g_encr_algo_s in ( 'aes128-cbc', 'aes192-cbc', 'aes256-cbc', 'rijndael256-cbc', 'rijndael-cbc@lysator.liu.se' ) then
            l_type := dbms_crypto.encrypt_aes + dbms_crypto.chain_cbc + dbms_crypto.pad_none;
        elsif g_encr_algo_s = '3des-cbc' then
            l_type := dbms_crypto.encrypt_3des + dbms_crypto.chain_cbc + dbms_crypto.pad_none;
        end if;

        l_tmp := dbms_crypto.decrypt(p_encr, l_type, g_key_cypher_s2c, g_iv_cypher_s2c);
        g_iv_cypher_s2c := p_encr;
        return l_tmp;
    end;
  --
    function read_packet return raw is
        ...
        l_block_size  pls_integer;
    begin
        if g_encr_algo_s in ( 'aes128-cbc', 'aes128-ctr', 'aes192-cbc', 'aes192-ctr', 'aes256-cbc',
                              'aes256-ctr', 'rijndael256-cbc', 'rijndael-cbc@lysator.liu.se' ) then
            l_block_size := 16;
        else
            l_block_size := 8;
        ...
        for i in 2..( 4 + l_packet_len ) / l_block_size loop
            l_tmp := decrypt(utl_tcp.get_raw(g_con, l_block_size));
            l_vbuf := l_vbuf || l_tmp;
            if length(l_vbuf) >= 32734  -- 32766 - 2 * (max blocksize)
             then
                if l_buf is null then
                    ...
                    l_buf := utl_raw.concat(l_buf,
                                            hextoraw(substr(l_vbuf,
                                                            1,
                                                            2 *(l_payload_len - utl_raw.length(l_buf)))));
                end if;
        ...
        g_seqn_s := g_seqn_s + 1;
        if g_seqn_s >= 4294967296 then
            g_seqn_s := 0;
        end if;
        if g_mac_algo_s in ( 'hmac-sha1', 'hmac-ripemd160' ) then
            l_mac := utl_tcp.get_raw(g_con, 20);
        elsif g_mac_algo_s = 'hmac-md5' then
            l_mac := utl_tcp.get_raw(g_con, 16);
        elsif g_mac_algo_s = 'hmac-sha2-256' then
            l_mac := utl_tcp.get_raw(g_con, 32);
        elsif g_mac_algo_s = 'hmac-sha2-512' then
            l_mac := utl_tcp.get_raw(g_con, 64);
        elsif g_mac_algo_s in ( 'hmac-md5-96', 'hmac-sha1-96' ) then
...
--
    function read_name_list (
        p_idx in out pls_integer,
        p_buf raw
    ) return tp_name_list is
        l_len       pls_integer;
        l_pos       pls_integer;
...
--
  ...
  --
    ...
    begin
        if
            p_bytes is not null
            and utl_raw.length(p_mpint) < p_bytes
        then
            append_string(p_buf,
                          utl_raw.concat(
                utl_raw.copies('00',
                               p_bytes - utl_raw.length(p_mpint)),
                p_mpint
            ));

        else
            if utl_raw.bit_and(
                utl_raw.substr(p_mpint, 1, 1),
                c_80
            ) = c_80 then
                append_string(p_buf,
                              utl_raw.concat('00', p_mpint));
            else
  ...
  --
    procedure append_name_list (
        p_buf       in out nocopy raw,
        p_name_list tp_name_list
    ) is
        l_tmp varchar2(32767);
    begin
        if p_name_list is null
           or p_name_list.count = 0 then
            p_buf := utl_raw.concat(p_buf, '00000000');
            return;
        end if;

        for i in p_name_list.first..p_name_list.last loop
            l_tmp := l_tmp
                     || ','
                     || p_name_list(i);
        end loop;
...
procedure handle_kex (
    p_buf         raw,
    p_fingerprint varchar2 := null,
    p_trust       boolean := null
);
  --
    procedure show_disconnect_msg (
        p_buf raw
    ) is
        l_reason number;
        l_idx    number;
        l_buf    raw(32767);
    begin
        if utl_raw.substr(p_buf, 1, 1) = ssh_msg_disconnect then
            l_idx := 2;
    ...
    begin
        loop
            ...
            case utl_raw.substr(p_buf, 1, 1)
                ...
                when p_msg3 then
                    exit;
                when ssh_msg_disconnect then
                    show_disconnect_msg(p_buf);
                    raise_application_error(-20020, 'Disconnected by server');
                when ssh_msg_ignore then
                    null;
                when ssh_msg_debug then
                    l_idx := 3;
                    get_string(l_idx, p_buf, l_buf);
                    info_msg(utl_i18n.raw_to_char(l_buf, 'AL32UTF8'));
                when ssh_msg_userauth_banner then
                    l_idx := 2;
                    get_string(l_idx, p_buf, l_buf);
                    info_msg(utl_i18n.raw_to_char(l_buf, 'AL32UTF8'));
                when ssh_msg_kexinit then
                    handle_kex(p_buf);
                else
                    null;
            end case;
  ...
  --
    ...
    begin
        close_connection;
        info_msg('try to connect to '
                 || p_host
                 || ', port '
                 || p_port);
        g_con := utl_tcp.open_connection(
            remote_host    => p_host,
            remote_port    => p_port,
        ...
        loop
            l_tmp := utl_tcp.get_raw(
                c    => g_con,
                len  => 4,
                peek => true
            );

            if l_tmp = hextoraw('5353482D') then
                v_s := utl_tcp.get_raw(
                    c    => g_con,
                    len  => 8,
                    peek => false
                ...
                or utl_raw.substr(v_s, 5, 4) = hextoraw('312E3939') -- 1.99
                 then
                    loop
                        ...
                        if l_tmp != hextoraw('0D') then
                            v_s := utl_raw.concat(v_s, l_tmp);
                        end if;

                    end loop;

                    l_dummy := utl_tcp.write_raw(g_con,
                                                 utl_raw.concat(v_c, '0D0A'));

                else
                    raise_application_error(-20001,
                                            'Invalid server version: '
                                            || utl_raw.cast_to_varchar2(utl_raw.substr(v_s, 5, 4)));
                end if;

                exit;
            else
                info_msg(utl_tcp.get_line(
                    c           => g_con,
                    remove_crlf => true
                ));
            end if;
  ...
  --
    procedure show_name_list (
        p_nl tp_name_list
    ) is
        l_str varchar2(32767);
    begin
        for i in 1..p_nl.count loop
            l_str := l_str
                     || ','
                     || p_nl(i);
        end loop;
  ...
  --
    procedure change_nl (
        ...
        procedure str2name_list (
            p_str  varchar2,
            p_list out tp_name_list
        ) is
            l_idx pls_integer;
    ...
    begin
        ...
        for i in 1..l_sort.count loop
            if
                l_sort(i) member of p_list
                and l_sort(i) not member of l_excl
            then
                l_tmp.extend;
        ...
        for i in 1..p_list.count loop
            if
                p_list(i) not member of l_tmp
                and p_list(i) not member of l_excl
            then
                l_tmp.extend;
  ...
  --
    ...
    --
        procedure add2name_list (
            p_name_list in out tp_name_list,
            p_val       varchar2
        ) is
        begin
            if hmac_sh256 is not null then
                p_name_list.extend;
        ...
        begin
            ...
            if l_tmp = hextoraw('7373682D647373') -- ssh-dss
             then
                ...
                l_u1 := demag(xmod(
                    rmul(
                        mag(dbms_crypto.hash(l_h, hash_sh1)),
                        mag(l_w)
                    ),
                    mag(l_q)
                ));
                ...
                l_v := lpad(
                    demag(xmod(
                        xmod(
                            rmul(
                                mag(powmod(l_g, l_u1, l_p)),
                                mag(powmod(l_y, l_u2, l_p))
                            ),
                            mag(l_p)
                        ),
                        mag(l_q)
            ...
            , hextoraw('7273612D736861322D353132') -- rsa-sha2-512
                ...
                if dbms_crypto.hash(l_h, l_hash_type) != utl_raw.substr(l_tmp, -l_hash_len) then
            ...
            , hextoraw('65636473612D736861322D6E69737470353231') -- ecdsa-sha2-nistp521
                ...
                if l_identifier != utl_raw.substr(l_tmp,
                                                  -utl_raw.length(l_identifier)) then
                    raise_application_error(-20019, 'ECDSA not OK');
                end if;

                get_named_curve(
                    utl_raw.cast_to_varchar2(l_identifier),
                    l_curve
                );
                get_string(l_idx, p_host_key, l_tmp);
                ...
                l_inv := powmod(
                    mag(l_ecdsa_s),
                    nsub(l_curve.group_order, 2),
                    l_curve.group_order
                );

                l_ecdsa_u1 := mulmod(
                    mag(dbms_crypto.hash(l_h, l_hash_type)),
                    l_inv,
                    l_curve.group_order
                );
                ...
                l_ecdsa_verify := add_point(
                    multiply_point(l_curve.generator, l_ecdsa_u1, l_curve),
                    multiply_point(l_ecdsa_w, l_ecdsa_u2, l_curve),
                    l_curve
                );

                if utl_raw.compare(
                    demag(l_ecdsa_verify.x),
                    ltrim(l_ecdsa_r, '0')
                ) != 0 then
            ...
            , hextoraw('7373682D6564343438')     -- ssh-ed448
                ...
                begin
                    ...
                    if l_signature_key != l_tmp then
                        raise_application_error(-20026, 'EdDSA not OK');
                    end if;
                    get_string(l_idx, p_signature, l_signature_blob);
                    get_named_ed_curve(
                        substr(
                            utl_raw.cast_to_varchar2(l_tmp),
                            5
                        ),
                        l_ed_curve
                    );
                    if l_ed_curve.b / utl_raw.length(l_signature_blob) != 4
                    or l_ed_curve.b / utl_raw.length(l_pub_key) != 8 then
                        debug_msg(l_ed_curve.b
                                  || ' '
                                  || utl_raw.length(l_signature_blob)
                                  || ' '
                                  || utl_raw.length(l_pub_key));
                ...
                raise_application_error(-20010,
                                        'unexpected public key algorithm ' || utl_raw.cast_to_varchar2(l_tmp));
            end if;
    ...
    begin
        ...
        first_kex_packet_follows := utl_raw.compare(
            utl_raw.substr(p_buf, l_idx, 1),
            '00'
        ) != 0;
    --
...
--    add2name_list( my_public_key_algorithms, 'ssh-ed25519' );
    ...
    --
        ...
        write_packet(i_c);
    --
        for i in my_encr_algo_client_to_server.first..my_encr_algo_client_to_server.last loop
            if my_encr_algo_client_to_server(i) member of encr_algo_client_to_server then
                l_encr_algo_c := my_encr_algo_client_to_server(i);
                exit;
            end if;
        end loop;

        if l_encr_algo_c is null then
    ...
    --
        for i in my_encr_algo_server_to_client.first..my_encr_algo_server_to_client.last loop
            if my_encr_algo_server_to_client(i) member of encr_algo_server_to_client then
                l_encr_algo_s := my_encr_algo_server_to_client(i);
                exit;
            end if;
        end loop;

        if l_encr_algo_s is null then
            error_msg('as_sftp algorithms');
        ...
        info_msg('using '
                 ...
                 || l_encr_algo_s);
    --
        for i in my_mac_algo_client_to_server.first..my_mac_algo_client_to_server.last loop
            if my_mac_algo_client_to_server(i) member of mac_algo_client_to_server then
                l_mac_algo_c := my_mac_algo_client_to_server(i);
                exit;
            end if;
        end loop;

        if l_mac_algo_c is null then
            raise_application_error(-20004, 'Could not find matching mac algorithm client to server');
        end if;
    --
        for i in my_mac_algo_server_to_client.first..my_mac_algo_server_to_client.last loop
            if my_mac_algo_server_to_client(i) member of mac_algo_server_to_client then
                l_mac_algo_s := my_mac_algo_server_to_client(i);
                exit;
            end if;
        end loop;

        if l_mac_algo_s is null then
        ...
        info_msg('using '
                 ...
                 || l_mac_algo_s);
    --
        for i in my_compr_algo_client_to_server.first..my_compr_algo_client_to_server.last loop
            if my_compr_algo_client_to_server(i) member of compr_algo_client_to_server then
                l_compr_algo_c := my_compr_algo_client_to_server(i);
                exit;
            end if;
        end loop;

        if l_compr_algo_c is null then
            raise_application_error(-20006, 'Could not find matching compression algorithm client to server');
        end if;
    --
        for i in my_compr_algo_server_to_client.first..my_compr_algo_server_to_client.last loop
            if my_compr_algo_server_to_client(i) member of compr_algo_server_to_client then
                l_compr_algo_s := my_compr_algo_server_to_client(i);
                exit;
            end if;
        end loop;

        if l_compr_algo_s is null then
            raise_application_error(-20007, 'Could not find matching compression algorithm server to client');
        end if;
    --    
        for i in my_public_key_algorithms.first..my_public_key_algorithms.last loop
            if my_public_key_algorithms(i) member of public_key_algorithms then
                l_public_key_algorithm := my_public_key_algorithms(i);
                exit;
            end if;
        end loop;

        if l_public_key_algorithm is null then
            error_msg('as_sftp algorithms');
    ...
    --
        for i in my_kex_algorithms.first..my_kex_algorithms.last loop
            if my_kex_algorithms(i) member of kex_algorithms then
  ...
  --
        if l_kex_algorithm = 'diffie-hellman-group1-sha1' then
        ...
        elsif l_kex_algorithm in ( 'diffie-hellman-group14-sha1', 'diffie-hellman-group14-sha256' ) then
            ...
            ;
            dh_g := '02';
            if l_kex_algorithm = 'diffie-hellman-group14-sha1' then
                l_hash_type := hash_sh1;
            else
                l_hash_type := hash_sh256;
            end if;
            ...
            l_buf := ssh_msg_kexdh_init;
        elsif l_kex_algorithm = 'diffie-hellman-group15-sha512' then
            ...
            l_hash_type := hash_sh512;
            l_buf := ssh_msg_kexdh_init;
        elsif l_kex_algorithm = 'diffie-hellman-group16-sha512' then
            ...
            l_hash_type := hash_sh512;
            l_buf := ssh_msg_kexdh_init;
        elsif l_kex_algorithm = 'diffie-hellman-group17-sha512' then
            ...
            l_hash_type := hash_sh512;
            l_buf := ssh_msg_kexdh_init;
        elsif l_kex_algorithm = 'diffie-hellman-group18-sha512' then
            ...
            l_hash_type := hash_sh512;
            l_buf := ssh_msg_kexdh_init;
        elsif l_kex_algorithm in ( 'diffie-hellman-group-exchange-sha1', 'diffie-hellman-group-exchange-sha256' ) then
            if l_kex_algorithm = 'diffie-hellman-group-exchange-sha1' then
                l_hash_type := hash_sh1;
            else
                l_hash_type := hash_sh256;
            end if;
        ...
        elsif l_kex_algorithm in ( 'ecdh-sha2-nistp256', 'ecdh-sha2-nistp384', 'ecdh-sha2-nistp521' ) then
            declare
                l_qc    raw(3999);
                l_qs    raw(3999);
                l_curve tp_ec_curve;
            ...
            begin
                ...
                if l_kex_algorithm = 'ecdh-sha2-nistp256' then
                    l_hash_type := hash_sh256;
                elsif l_kex_algorithm = 'ecdh-sha2-nistp384' then
                    l_hash_type := hash_sh384;
                elsif l_kex_algorithm = 'ecdh-sha2-nistp521' then
                    l_hash_type := hash_sh512;
                end if;

                l_xxx := mag(dbms_crypto.randombytes(4));
                get_named_curve(
                    substr(l_kex_algorithm, -8),
                    l_curve
                );
                l_pb := multiply_point(l_curve.generator, l_xxx, l_curve);
                l_qc := utl_raw.concat('04',
                                       lpad(
                                                   demag(l_pb.x),
                                                   2 * l_curve.nlen,
                                                   '0'
  ...
  --
        if l_kex_algorithm not in ( 'ecdh-sha2-nistp256', 'ecdh-sha2-nistp384', 'ecdh-sha2-nistp521' ) then
            ...
            case utl_raw.substr(l_buf, 1, 1)
                when ssh_msg_kexdh_reply then
                    ...
                    l_h := dbms_crypto.hash(l_tmp, l_hash_type);
                    get_string(l_idx, l_buf, l_s);
                when ssh_msg_kex_dh_gex_reply then
                    l_idx := 2;
                    get_string(l_idx, l_buf, k_s);
                    get_mpint(l_idx, l_buf, l_f); -- f
                    l_k := powmod(l_f, l_x, dh_p);
    ...
    --
        if l_host_fingerprint = lower(p_fingerprint)
           ...
           or p_trust then
            ...
            using (
                select
                    upper(g_con.remote_host) p_host
                from
                    dual
            ) on ( host = p_host )
            ...
            if c_known_hosts%notfound then
                r_known_hosts.fingerprint := null;
            end if;
            close c_known_hosts;
            if l_host_fingerprint = r_known_hosts.fingerprint then
                null; -- OK
            else
    ...
    --
        ...
        ,
                                      case
                                          when g_encr_algo_c in('3des-cbc', '3des-ctr') then
                                              8
                                          else 16
                                      end
        );
        ...
        ,
                                      case
                                          when g_encr_algo_s in('3des-cbc', '3des-ctr') then
                                              8
                                          else 16
                                      end
        );

        g_iv_cypher_s2c_ctr := to_number(g_iv_cypher_s2c,
                                         rpad('0', 32, 'X'));
        ...
        ,
                                       case g_encr_algo_c
                                           ...
                                           when '3des-ctr' then
                                               24
                                           when 'aes192-cbc' then
                                               24
                                           when 'aes192-ctr' then
                                               24
                                           when 'aes256-cbc' then
                                               32
                                           when 'aes256-ctr' then
                                               32
                                           when 'arcfour256' then
                                               32
                                           when 'rijndael256-cbc' then
                                               32
                                           when 'rijndael-cbc@lysator.liu.se' then
                                               32
                                           when 'rijndael192-cbc' then
                                               24
                                           else 16
                                       end
        );
        ...
        ,
                                    case
                                        when g_mac_algo_c in('hmac-sha1', 'hmac-sha1-96', 'hmac-ripemd160') then
                                            20
                                        when g_mac_algo_c in('hmac-sha2-256') then
                                            32
                                        when g_mac_algo_c in('hmac-sha2-512') then
                                            64
                                        else 16
                                    end
        );
        ...
        ,
                                    case
                                        when g_mac_algo_s in('hmac-sha1', 'hmac-sha1-96', 'hmac-ripemd160') then
                                            20
                                        when g_mac_algo_s in('hmac-sha2-256') then
                                            32
                                        when g_mac_algo_s in('hmac-sha2-512') then
                                            64
                                        else 16
                                    end
        );
  ...
  --
    function do_auth (
        ...
        c_oid        raw(1) := '06';
        c_sequence   raw(1) := '30';
        type tp_pk_parameters is
            table of raw(3999) index by pls_integer;
    --
        function write_pk (
            p_algo      varchar2,
            p_blob      raw,
        ...
        begin
            ...
            append_string(l_buf,
                          utl_i18n.string_to_raw(p_user, 'AL32UTF8'));
            append_string(l_buf,
                          utl_i18n.string_to_raw('ssh-connection', 'US7ASCII'));
            append_string(l_buf,
                          utl_i18n.string_to_raw('publickey', 'US7ASCII'));
            append_boolean(l_buf, p_signature is not null);
            append_string(l_buf,
                          utl_i18n.string_to_raw(p_algo, 'US7ASCII'));
            ...
            case utl_raw.substr(l_buf4, 1, 1)
                when ssh_msg_userauth_success then
                    info_msg(p_algo || ' public key OK');
                    l_rv := true;
                when ssh_msg_userauth_failure then
                    info_msg(p_algo || ' public key not OK');
                when ssh_msg_userauth_pk_ok then
                    info_msg('server accepts '
                             || p_algo
                             || ' public key');
                    l_pk_ok := true;
    ...
    --
        function write_rsa_pk (
            p_pk_parameters tp_pk_parameters
        ) return boolean is
            l_rv   boolean;
            l_buf3 raw(3999);
            l_buf5 raw(3999);
        begin
            if p_pk_parameters.count = 0 then
                return false;
            end if;
            l_buf2 := null;
            append_string(l_buf2,
                          utl_i18n.string_to_raw('ssh-rsa', 'US7ASCII'));
            append_mpint(l_buf2,
                         p_pk_parameters(2));
            append_mpint(l_buf2,
                         p_pk_parameters(1));
            ...
            l_buf3 := powmod(
                utl_raw.concat('0001',
                               utl_raw.copies('FF',
                                              utl_raw.length(p_pk_parameters(1)) - 38 - case
                                  when utl_raw.substr(
                                      p_pk_parameters(1),
                                      1,
                                      1
    ...
    --
        ...
        begin
            l_buf2 := null;
            append_string(l_buf2,
                          utl_i18n.string_to_raw('ssh-dss', 'US7ASCII'));
            append_mpint(l_buf2,
                         p_pk_parameters(1));
            append_mpint(l_buf2,
                         p_pk_parameters(2));
            ...
            loop
                ...
                l_idx := utl_raw.compare(l_k,
                                         p_pk_parameters(2));
                exit when
                    utl_raw.substr(l_k, -3) != '000000'
                    and utl_raw.substr(l_k, l_idx, 1) < utl_raw.substr(
                        p_pk_parameters(2),
                        l_idx,
                        1
                    );
            ...
            l_s := demag(l_dummy);
            l_s := utl_raw.overlay(l_s,
                                   utl_raw.copies('00', 20),
                                   21 - utl_raw.length(l_s));

            l_r := utl_raw.overlay(l_r,
                                   utl_raw.copies('00', 20),
                                   21 - utl_raw.length(l_r));

            l_buf3 := null;
            append_string(l_buf3,
                          utl_i18n.string_to_raw('ssh-dss', 'US7ASCII'));
            append_string(l_buf3,
                          utl_raw.concat(l_r, l_s));
    ...
    --
        ...
        begin
            ...
            l_buf2 := null;
            l_algo := utl_raw.concat('65636473612D736861322D',
                                     p_pk_parameters(1)); -- ecdsa-sha2-
            ...
            append_string(l_buf2,
                          p_pk_parameters(2));
            l_rv := write_pk(
                utl_i18n.raw_to_char(l_algo, 'US7ASCII'),
                l_buf2,
                null
            );
            ...
            get_named_curve(
                utl_raw.cast_to_varchar2(p_pk_parameters(1)),
                l_curve
            );
            append_string(l_buf3, g_session_id);
            append_byte(l_buf3, l_buf);
            ...
            l_s := mulmod(
                radd(
                    mag(dbms_crypto.hash(l_buf3, l_hash_type)),
                    mulmod(
                        mag(p_pk_parameters(3)),
                        l_r,
                        l_curve.group_order
                    )
                ),
    ...
    --
        ...
        begin
            ...
            if l_len > 127 then
                ...
                p_ind := p_ind + 1;
                l_len := to_number(
                    utl_raw.substr(p_key, p_ind, l_tmp),
                    rpad('x', 2 * l_tmp, 'x')
                );
    ...
    --
        procedure check_starting_sequence (
            p_key raw,
            p_ind in out pls_integer
        ) is
            l_len pls_integer;
        begin
            p_ind := 1;
            if utl_raw.substr(p_key, p_ind, 1) != c_sequence then
    ...
    --
        function get_bytes (
            ...
            p_ind                    in out pls_integer,
            p_msg                    varchar2 := '',
            p_skip_enclosing_context boolean := true
        ) return raw is
            l_first raw(1);
            l_len   pls_integer;
        begin
            l_first := utl_raw.substr(p_key, p_ind, 1);
            if l_first != p_type then
                if
                    p_skip_enclosing_context
                    and utl_raw.bit_and(l_first, 'C0') = '80'
                then
                    l_len := get_len(p_key, p_ind);
    ...
    --
      ...
      -- skip unused bits value, assume always 0
            return utl_raw.substr(
                get_bytes(c_bit_string, p_key, p_ind, p_msg),
                2
            );
        end;
    --
    ...
    --
        ...
        begin
            ...
            then
                ...
                l_tmp := utl_raw.concat(p_pw,
                                        utl_raw.substr(l_iv, 1, 8));
                ...
                l_rv := dbms_crypto.decrypt(
                    utl_encode.base64_decode(utl_raw.cast_to_raw(l_key)),
    ...
    --
        function parse_rsa_private_key (
            p_key varchar2,
            p_pw  raw
        ) return boolean is
            l_rv            boolean;
        ...
        begin
            ...
            or substr(p_key, -19) != 'END RSA PRIVATE KEY' ) then
                return false;
            end if;

            l_rv := parse_der_rsa_key(
                decrypt_private_key(p_key, p_pw),
                l_pk_parameters
            );
            if l_rv then
    ...
    --
        function parse_dsa_private_key (
            p_key varchar2,
            p_pw  raw
        ) return boolean is
            l_rv            boolean;
        ...
        begin
            ...
            or substr(p_key, -19) != 'END DSA PRIVATE KEY' ) then
                return false;
            end if;

            l_rv := parse_der_dsa_key(
                decrypt_private_key(p_key, p_pw),
                l_pk_parameters
            );
            if l_rv then
    ...
    --
        ...
        begin
            ...
            or substr(p_key, -18) != 'END EC PRIVATE KEY' ) then
                return false;
            end if;

            l_rv := parse_der_ec_key(
                decrypt_private_key(p_key, p_pw),
                l_pk_parameters
            );
            if l_rv then
    ...
    --
        function parse_openssh_private_key (
            p_key varchar2,
            p_pw  raw
        ) return boolean is
        ...
        begin
            ...
            or substr(p_key, -23) != 'END OPENSSH PRIVATE KEY' ) then
                return false;
            end if;
      -- http://dnaeon.github.io/openssh-private-key-binary-format/
            ...
            l_idx := l_idx + 4;
            if l_len between 1 and 32767 then
                l_ciphername := utl_raw.cast_to_varchar2(utl_raw.substr(l_tmp, l_idx, l_len));
            end if;

            if l_ciphername is null
               or l_ciphername not in ( 'none', 'aes256-ctr' ) then
                return false;
            end if;
            ...
            if l_kdfname is null
               or l_kdfname not in ( 'none', 'bcrypt' ) then
                return false;
            end if;
            ...
            if l_kdfname = 'bcrypt' then
                return false;
            end if;
            if l_ciphername = 'aes256-ctr' then
                l_iv_ctr := to_number(l_iv,
                                      rpad('0', 32, 'X'));
                for i in 0..l_len / 16 - 1 loop
                    l_pk_bytes := utl_raw.concat(l_pk_bytes,
                                                 utl_raw.bit_xor(
                                                                 utl_raw.substr(l_tmp, l_idx + i * 16, 16),
                                                                 dbms_crypto.encrypt(
                                                                             substr(
                                                                                 to_char(l_iv_ctr, 'FM0XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'
                                                                                 ),
                                                                                 -32
                                                                             ),
                                                                             dbms_crypto.encrypt_aes + dbms_crypto.chain_cbc + dbms_crypto.pad_none
                    ...
                    l_iv_ctr := l_iv_ctr + 1;
                    if l_iv_ctr > 340282366920938463463374607431768211455 then
                        l_iv_ctr := 0;
                    end if;
                end loop;

            elsif l_ciphername = 'none' then
                l_pk_bytes := utl_raw.substr(l_tmp, l_idx);
            end if;

            if utl_raw.substr(l_pk_bytes, 1, 4) != utl_raw.substr(l_pk_bytes, 5, 4) then
                return false;
            end if;
            ...
            l_idx := l_idx + 4;
            if l_len between 1 and 3999 then
                l_keytype := utl_raw.cast_to_varchar2(utl_raw.substr(l_pk_bytes, l_idx, l_len));
            end if;

            l_idx := l_idx + l_len;
            debug_msg('keytype: ' || l_keytype);
            if l_keytype = 'ssh-rsa' then
                l_cnt := 3;
            elsif l_keytype = 'ssh-dss' then
                l_cnt := 5;
            elsif l_keytype in ( 'ecdsa-sha2-nistp256', 'ecdsa-sha2-nistp384', 'ecdsa-sha2-nistp521' ) then
                l_cnt := 4;
            elsif l_keytype in ( 'ssh-ed25519', 'ssh-ed448' ) then
                l_cnt := 2;
            end if;
    ...
    --
        ...
        begin
            ...
            if instr(
                ...
                l_rv := parse_dsa_private_key(l_key,
                                              utl_raw.cast_to_raw(p_passphrase));
            elsif instr(
                substr(l_key, 1, 50),
                'RSA'
            ) > 0 then
                l_rv := parse_rsa_private_key(l_key,
                                              utl_raw.cast_to_raw(p_passphrase));
            elsif instr(
                substr(l_key, 1, 50),
                'OPENSSH PRIVATE'
            ) > 0 then
                l_rv := parse_openssh_private_key(l_key,
                                                  utl_raw.cast_to_raw(p_passphrase));
            elsif instr(
                substr(l_key, 1, 50),
                ' EC '
            ) > 0 then
                l_rv := parse_ec_private_key(l_key,
                                             utl_raw.cast_to_raw(p_passphrase));
            else
                l_rv := false;
            end if;
  ...
  --
    begin
        ...
        append_string(l_buf,
                      utl_i18n.string_to_raw('ssh-userauth', 'US7ASCII'));
        write_packet(l_buf);
        ...
        append_string(l_buf,
                      utl_i18n.string_to_raw('ssh-connection', 'US7ASCII'));
        append_string(l_buf,
                      utl_i18n.string_to_raw('none', 'US7ASCII'));
        write_packet(l_buf);
        ...
        append_string(l_buf,
                      utl_i18n.string_to_raw('ssh-connection', 'US7ASCII'));
        append_string(l_buf,
                      utl_i18n.string_to_raw('password', 'US7ASCII'));
        append_boolean(l_buf, false);
        append_string(l_buf,
                      utl_i18n.string_to_raw(p_pw, 'AL32UTF8'));
        write_packet(l_buf);
        read_until(l_buf, ssh_msg_userauth_success, ssh_msg_userauth_failure);
        case utl_raw.substr(l_buf, 1, 1)
            when ssh_msg_userauth_success then
        ...
        if p_priv_key is null then
            return false;
        end if;
        return parse_private_key(p_priv_key, p_passphrase);
    end;
  --
    procedure write_fxp_message (
        p_type    raw,
        p_payload raw
    ) is
        l_buf raw(32767);
    begin
        ...
        append_string(l_buf,
                      utl_raw.concat(
            to_char(1 + utl_raw.length(p_payload),
                    'fm0XXXXXXX'),
            p_type,
            p_payload
        ));
...
--
    ...
    begin
        ...
        if l_channel = g_ssh_channel.my_channel then
            get_string(l_idx, l_buf, p_buf);
            g_ssh_channel.cur_window_size := g_ssh_channel.cur_window_size - utl_raw.length(l_buf);
            if g_ssh_channel.cur_window_size < g_ssh_channel.max_window_size / 2 then
          ...
          -- skip length attribute
                    ...
                    while utl_raw.length(p_buf) < l_len loop
  ...
  --
    function open_sftp return boolean is
    ...
    begin
        ...
        append_string(l_buf,
                      utl_i18n.string_to_raw('session', 'US7ASCII'));
        ...
        if utl_raw.substr(l_buf, 1, 1) = ssh_msg_channel_open_failure then
            l_idx := 2;
            ...
            append_int32(l_buf, g_ssh_channel.server_channel);
            append_string(l_buf,
                          utl_i18n.string_to_raw('subsystem', 'US7ASCII'));
            append_boolean(l_buf, true);
            append_string(l_buf,
                          utl_i18n.string_to_raw('sftp', 'US7ASCII'));
            write_packet(l_buf);
            read_until(l_buf, ssh_msg_channel_success, ssh_msg_channel_failure);
            if utl_raw.substr(l_buf, 1, 1) = ssh_msg_channel_failure then
                ...
                loop
                    ...
                    if utl_raw.substr(l_buf, 1, 1) = ssh_fxp_version then
                        l_idx := 2;
                        get_int32(l_idx, l_buf, l_dummy);
                        if l_dummy = 3 then
                            ...
                            error_msg('sftp not openend, server version: '
                                      || l_dummy
                                      || ' not supported');
                            l_rv := false;
                        end if;
  ...
  --
    ...
    begin
        ...
        append_string(l_buf,
                      utl_i18n.string_to_raw(p_path, 'AL32UTF8'));
        write_fxp_message(ssh_fxp_realpath, l_buf);
        loop
            read_fxp_message(l_buf);
            if utl_raw.substr(l_buf, 1, 1) = ssh_fxp_name then
                l_idx := 2;
                get_int32(l_idx, l_buf, l_dummy);
        -- check l_dummy = l_fxp_id?
                ...
                exit;
            elsif utl_raw.substr(l_buf, 1, 1) = ssh_fxp_status then
                l_idx := 2;
                get_int32(l_idx, l_buf, l_dummy);
        -- check l_dummy = l_fxp_id?
                get_int32(l_idx, l_buf, l_dummy);
  ...
  --
    function pwd return varchar2 is
    begin
  ...
  --
    ...
    begin
        ...
        append_int32(l_buf, l_fxp_id);
        append_string(l_buf,
                      utl_i18n.string_to_raw(i_path, 'AL32UTF8'));
        ...
        loop
            ...
            if utl_raw.substr(l_buf, 1, 1) = ssh_fxp_status then
                ...
                loop
                    ...
                    if utl_raw.substr(l_buf, 1, 1) = ssh_fxp_data then
                        l_idx := 2;
                        get_int32(l_idx, l_buf, l_dummy);
                        get_string(l_idx, l_buf, l_buf2);
                        dbms_lob.writeappend(i_file,
                                             utl_raw.length(l_buf2),
                                             l_buf2);
                    end if;
        ...
        --
                ...
                if dbms_lob.istemporary(i_file) = 0 then
  ...
  --  
    ...
    begin
        ...
        for i in 0..trunc((dbms_lob.getlength(l_file) - 1) / l_len) loop
            utl_file.put_raw(l_fh,
                             dbms_lob.substr(l_file, l_len, i * l_len + 1));
        end loop;
    ...
    begin
        ...
        loop
            ...
            if utl_raw.substr(l_buf, 1, 1) = ssh_fxp_status then
                ...
                get_string(l_idx, l_buf, l_fxp_handle);
                l_file_offset := 0;
                for i in 1..ceil(dbms_lob.getlength(i_file) / l_len) loop
                    l_buf := null;
                    l_fxp_id := 252;
                    append_int32(l_buf, l_fxp_id);
                    append_string(l_buf, l_fxp_handle);
                    append_int64(l_buf, l_file_offset);
                    append_string(l_buf,
                                  dbms_lob.substr(i_file, l_len, l_file_offset + 1));
                    l_file_offset := l_file_offset + l_len;
...
--
                loop
                    ...
                    for i in 1..l_dummy loop
                        ...
                        if bitand(l_flags, ssh_filexfer_attr_size) = ssh_filexfer_attr_size then
                            get_int64(l_idx, l_buf, l_dir_line.file_size);
                        end if;

                        if bitand(l_flags, ssh_filexfer_attr_uidgid) = ssh_filexfer_attr_uidgid then
                            get_int32(l_idx, l_buf, l_dir_line.uid);
                            get_int32(l_idx, l_buf, l_dir_line.gid);
                        end if;

                        if bitand(l_flags, ssh_filexfer_attr_permissions) = ssh_filexfer_attr_permissions then
                            get_int32(l_idx, l_buf, l_dir_line.perm);
                        end if;

                        if bitand(l_flags, ssh_filexfer_attr_acmodtime) = ssh_filexfer_attr_acmodtime then
                            get_int32(l_idx, l_buf, l_dummy);
                        ...
                        if bitand(l_flags, ssh_filexfer_attr_extended) = ssh_filexfer_attr_extended then
                            get_int32(l_idx, l_buf, l_cnt);
                            for c in 1..l_cnt loop
                                get_string(l_idx, l_buf, l_buf2);
                                get_string(l_idx, l_buf, l_buf2);
                            end loop;
  ...
  --
    ...
    begin
        ...
        if do_auth(i_user, i_password, i_priv_key, i_passphrase) then
            info_msg('logged in');
            if open_sftp then
                info_msg('sftp open');
            else
  ...
  --
    procedure open_connection (
        ...
        l_buf raw(32767);
    begin
        if setup_connection(p_host, p_port) then
  ...
  --
    ...
    begin
        open_connection(i_host, i_port, null, null, i_excluded_kex_algos,
                        i_preferred_kex_algos, i_excluded_encr_algos, i_preferred_encr_algos, i_excluded_pkey_algos, i_preferred_pkey_algos
                        );
    end;
  --
    procedure open_connection (
    ...
    begin
        open_connection(i_host, i_port, null, i_trust_server, i_excluded_kex_algos,
                        i_preferred_kex_algos, i_excluded_encr_algos, i_preferred_encr_algos, i_excluded_pkey_algos, i_preferred_pkey_algos
                        );
    end;
  --
    ...
    begin
        open_connection(i_host, i_port, i_fingerprint, null, i_excluded_kex_algos,
                        i_preferred_kex_algos, i_excluded_encr_algos, i_preferred_encr_algos, i_excluded_pkey_algos, i_preferred_pkey_algos
                        );
    end;
  --
    procedure close_connection is
        l_buf raw(32767);
    begin
        if g_ssh_channel.server_channel is not null then
            l_buf := ssh_msg_channel_close;
            append_int32(l_buf, g_ssh_channel.server_channel);
            write_packet(l_buf);
      -- a lot of servers don't handle this according to specs and don't send this message
        ...
        if g_con.remote_host is not null then
