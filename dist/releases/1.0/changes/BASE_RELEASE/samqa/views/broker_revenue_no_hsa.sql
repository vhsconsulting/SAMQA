...
-- changeset SAMQA:1754374169419 stripComments:false logicalFilePath:BASE_RELEASE\samqa\views\broker_revenue_no_hsa.sql runAlways:false runOnChange:false replaceIfExists:true failOnError:true
-- sqlcl_snapshot src/database/samqa/views/broker_revenue_no_hsa.sql:null:6bf48218d0c046ef859fbc8cd2a3e1e024648367:create

create or replace force editionable view samqa.broker_revenue_no_hsa (
    acc_id,
    account_type,
    broker_id,
    salesrep_id,
   ...
   /* AND (   -- approved in the period
          ( trunc(i.approved_date) BETWEEN trunc(TO_DATE(:p90_start_date, 'MM/DD/YYYY')) AND trunc(TO_DATE(:p90_end_date, 'MM/DD/YYYY')) 
              AND trunc(i.start_date)    <= trunc(TO_DATE(:p90_end_date, 'MM/DD/YYYY')) 
          )
          OR         -- approved earlier but start in the period
          ( trunc(i.start_date)    BETWEEN trunc(TO_DATE(:p90_start_date, 'MM/DD/YYYY')) AND trunc(TO_DATE(:p90_end_date, 'MM/DD/YYYY'))
              AND trunc(i.approved_date) < trunc(TO_DATE(:p90_start_date, 'MM/DD/YYYY')) 
          ) 
        ) */
