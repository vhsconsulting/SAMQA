...
-- changeset SAMQA:1754373926947 stripComments:false logicalFilePath:BASE_RELEASE\samqa\functions\broker_rev_rnw_fn.sql runAlways:false runOnChange:false replaceIfExists:true failOnError:true
-- sqlcl_snapshot src/database/samqa/functions/broker_rev_rnw_fn.sql:null:ec20aa5e831bd44d5e6802c211db0cebf9296d5d:create

create or replace function samqa.broker_rev_rnw_fn (
    p_mon_period        number,
    p_num               number,
    p_hasrev            number,
    p_result            number,
...
) -- 20240927 add , p_filter_start_date date, p_filter_end_date date 
...
begin
    ...
    elsif p_mon_period = 3 then -- section --20260927  add section
        l_cond := q'{
    ...
    l_sql := 'select * from BROKER_ACCOUNT_NOHSA_RNW_REV_MV where ' || l_cond;
    l_sql := l_sql
             || ' UNION ALL '
             || 'select * from BROKER_ACCOUNT_HSA_RNW_REV_MV where '
             || l_cond2;
