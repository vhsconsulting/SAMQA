...
-- changeset SAMQA:1754373926886 stripComments:false logicalFilePath:BASE_RELEASE\samqa\functions\broker_rev_by_broker_fn.sql runAlways:false runOnChange:false replaceIfExists:true failOnError:true
-- sqlcl_snapshot src/database/samqa/functions/broker_rev_by_broker_fn.sql:null:06a740ee24a6106ad71ca787340cd0b896259c4b:create

create or replace function samqa.broker_rev_by_broker_fn (
    p_broker_id         number,
    p_mon_period        number,
    p_num               number,
    p_result            number,
...
begin
    ...
    l_sql := 'select * from BROKER_ACCOUNT_NOHSA_REV_MV where broker_id = '
             || p_broker_id
             || ' and ('
             || l_cond
             || ')';
    l_sql := l_sql
             || ' UNION ALL '
             || 'select * from BROKER_ACCOUNT_HSA_REV_MV where broker_id = '
             || p_broker_id
             || ' and ('
             || l_cond2
             || ')';
